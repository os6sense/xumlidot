...
= PRERELEASE VERSION (i.e. work in progress/not production ready)
  - dot output works quite nicely.
  - xmi output is duplicating base classes (next issue to fix).
  - can parse rails so *should* work on you project - please let me know if it doesn't
...
--[no]-consolodate (TODO: Use only a single path between each class - will
become the default in order to reduce the number of paths that have to be
drawn)
...
- SPECS!
- fix errors seen in some codebases (e.g. rails)
- add an ingore option: e.g. it would be nice to ignore rails test classes.
